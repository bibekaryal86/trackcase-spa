...
import { CourtCaseResponse, CourtCaseSchema } from '../types/court_cases.data.types'
    ...
    try {
      ...
      if (isForceFetch || courtCasesInStore.length === 0) {
        ...
        if (courtCasesResponse.detail) {
          ...
          dispatch(courtCasesSuccess(COURT_CASES_RETRIEVE_SUCCESS, '', courtCasesResponse.court_cases))
        }
      } else {
        courtCasesResponse = {
          court_cases: courtCasesInStore,
        }
        dispatch(courtCasesSuccess(COURT_CASES_RETRIEVE_SUCCESS, '', courtCasesResponse.court_cases))
      }
    } catch (error) {
  ...
  return async (dispatch: React.Dispatch<GlobalDispatch>, getStore: () => GlobalState): Promise<void> => {
    ...
    try {
      ...
      if (courtCaseResponse.detail) {
        ...
        dispatch(courtCaseSelect(courtCaseResponse.court_cases[0]))
      }
    } catch (error) {
...
const getRequestBody = (court_case: CourtCaseSchema) => {
  return {
    case_type_id: court_case.case_type_id,
