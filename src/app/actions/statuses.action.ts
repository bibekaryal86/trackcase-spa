...
import {
  STATUSES_COMPLETE,
  STATUSES_READ_FAILURE,
  STATUSES_READ_REQUEST,
  STATUSES_READ_SUCCESS,
...
export const getStatusesList = () => {
  return async (dispatch: React.Dispatch<GlobalDispatch>, getStore: () => GlobalState): Promise<void> => {
    dispatch(statusesRequest(STATUSES_READ_REQUEST))
    ...
    try {
      ...
      dispatch(statusesSuccess(STATUSES_READ_SUCCESS, '', statusSchema))
    } catch (error) {
      console.log('Get Statuses Error: ', error)
      dispatch(statusesFailure(STATUSES_READ_FAILURE, SOMETHING_WENT_WRONG))
    } finally {
