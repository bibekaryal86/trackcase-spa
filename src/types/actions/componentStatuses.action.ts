...
export const getComponentStatuses = (isForceFetch: boolean = false) => {
  return async (dispatch: React.Dispatch<GlobalDispatch>, getStore: () => GlobalState): Promise<void> => {
    dispatch(componentStatusesRequest(COMPONENT_STATUS_RETRIEVE_REQUEST))

    try {
      let componentStatusResponse: ComponentStatusResponse
      const componentStatusesInStore: ComponentStatusSchema[] = getStore().componentStatus.data
      ...
      if (isForceFetch || componentStatusesInStore.length === 0) {
        ...
        if (componentStatusResponse.detail) {
          dispatch(
            componentStatusesFailure(COMPONENT_STATUS_RETRIEVE_FAILURE, getErrMsg(componentStatusResponse.detail)),
          )
        } else {
          dispatch(componentStatusesSuccess(COMPONENT_STATUS_RETRIEVE_SUCCESS, '', componentStatusResponse.data))
        }
      } else {
        dispatch(componentStatusesSuccess(COMPONENT_STATUS_RETRIEVE_SUCCESS, '', componentStatusesInStore))
      }
    } catch (error) {
      console.log('Get ComponentStatuses Error: ', error)
      dispatch(componentStatusesFailure(COMPONENT_STATUS_RETRIEVE_FAILURE, SOMETHING_WENT_WRONG))
    } finally {
      dispatch(componentStatusesComplete())
    }
  }
...
const componentStatusesComplete = () => ({
  type: COMPONENT_STATUS_COMPLETE,
})
