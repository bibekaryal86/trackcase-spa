...
import { FormTypeResponse, FormTypeSchema } from '../types/ref_types.data.types'
...
export const getFormTypes = (isForceFetch: boolean = false) => {
  return async (dispatch: React.Dispatch<GlobalDispatch>, getStore: () => GlobalState): Promise<void> => {
    ...
    try {
      ...
      const formTypesInStore: FormTypeSchema[] = getStore().formTypes.form_types
      ...
      if (isForceFetch || formTypesInStore.length === 0) {
        ...
        if (formTypeResponse.detail) {
          ...
          dispatch(formTypesSuccess(FORM_TYPES_RETRIEVE_SUCCESS, '', formTypeResponse.form_types))
        }
      } else {
        formTypeResponse = {
          form_types: formTypesInStore,
        }
        dispatch(formTypesSuccess(FORM_TYPES_RETRIEVE_SUCCESS, '', formTypeResponse.form_types))
      }
    } catch (error) {
