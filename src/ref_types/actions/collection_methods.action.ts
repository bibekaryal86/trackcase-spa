...
import { CollectionMethodResponse, CollectionMethodSchema } from '../types/ref_types.data.types'
...
export const getCollectionMethods = (isForceFetch: boolean = false) => {
  return async (dispatch: React.Dispatch<GlobalDispatch>, getStore: () => GlobalState): Promise<void> => {
    ...
    try {
      ...
      const collectionMethodsInStore: CollectionMethodSchema[] = getStore().collectionMethods.collection_methods
        ...
        if (collectionMethodResponse.detail) {
          ...
          dispatch(
            collectionMethodsSuccess(
              COLLECTION_METHODS_RETRIEVE_SUCCESS,
              '',
              collectionMethodResponse.collection_methods,
            ),
          )
        }
      } else {
        collectionMethodResponse = {
          collection_methods: collectionMethodsInStore,
        }
        dispatch(
          collectionMethodsSuccess(
            COLLECTION_METHODS_RETRIEVE_SUCCESS,
