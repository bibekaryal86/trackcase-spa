...
import { HearingTypeResponse, HearingTypeSchema } from '../types/ref_types.data.types'
...
export const getHearingTypes = (isForceFetch: boolean = false) => {
  return async (dispatch: React.Dispatch<GlobalDispatch>, getStore: () => GlobalState): Promise<void> => {
    ...
    try {
      ...
      const hearingTypesInStore: HearingTypeSchema[] = getStore().hearingTypes.hearing_types
      ...
      if (isForceFetch || hearingTypesInStore.length === 0) {
        ...
        if (hearingTypeResponse.detail) {
          ...
          dispatch(hearingTypesSuccess(HEARING_TYPES_RETRIEVE_SUCCESS, '', hearingTypeResponse.hearing_types))
        }
      } else {
        hearingTypeResponse = {
          hearing_types: hearingTypesInStore,
        }
        dispatch(hearingTypesSuccess(HEARING_TYPES_RETRIEVE_SUCCESS, '', hearingTypeResponse.hearing_types))
      }
    } catch (error) {
