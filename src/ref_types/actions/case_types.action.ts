...
import { CaseTypeResponse, CaseTypeSchema } from '../types/ref_types.data.types'
...
export const getCaseTypes = (isForceFetch: boolean = false) => {
  return async (dispatch: React.Dispatch<GlobalDispatch>, getStore: () => GlobalState): Promise<void> => {
    ...
    try {
      ...
      const caseTypesInStore: CaseTypeSchema[] = getStore().caseTypes.case_types
      ...
      if (isForceFetch || caseTypesInStore.length === 0) {
        ...
        if (caseTypeResponse.detail) {
          ...
          dispatch(caseTypesSuccess(CASE_TYPES_RETRIEVE_SUCCESS, '', caseTypeResponse.case_types))
        }
      } else {
        caseTypeResponse = {
          case_types: caseTypesInStore,
        }
        dispatch(caseTypesSuccess(CASE_TYPES_RETRIEVE_SUCCESS, '', caseTypeResponse.case_types))
      }
    } catch (error) {
