...
import { TaskTypeResponse, TaskTypeSchema } from '../types/ref_types.data.types'
...
export const addTaskType = (name: string, description: string) => {
  return async (dispatch: React.Dispatch<GlobalDispatch>): Promise<void> => {
    ...
    try {
      const urlPath = getEndpoint(process.env.TASK_TYPE_CREATE_ENDPOINT as string)
...
export const getTaskTypes = (isForceFetch: boolean = false) => {
  return async (dispatch: React.Dispatch<GlobalDispatch>, getStore: () => GlobalState): Promise<void> => {
    ...
    try {
      ...
      const taskTypesInStore: TaskTypeSchema[] = getStore().taskTypes.task_types

      if (isForceFetch || taskTypesInStore.length === 0) {
        const urlPath = getEndpoint(process.env.TASK_TYPES_RETRIEVE_ENDPOINT as string)
        const options: Partial<FetchOptions> = {
          method: 'GET',
        }
        ...
        if (taskTypeResponse.detail) {
          ...
          dispatch(taskTypesSuccess(TASK_TYPES_RETRIEVE_SUCCESS, '', taskTypeResponse.task_types))
        }
      } else {
        taskTypeResponse = {
          task_types: taskTypesInStore,
        }
        dispatch(taskTypesSuccess(TASK_TYPES_RETRIEVE_SUCCESS, '', taskTypeResponse.task_types))
      }
    } catch (error) {
...
export const editTaskType = (id: number, name: string, description: string) => {
  return async (dispatch: React.Dispatch<GlobalDispatch>): Promise<void> => {
    ...
    try {
      const urlPath = getEndpoint(process.env.TASK_TYPE_UPDATE_ENDPOINT as string)
      const options: Partial<FetchOptions> = {
        method: 'PUT',
...
export const deleteTaskType = (id: number) => {
  return async (dispatch: React.Dispatch<GlobalDispatch>): Promise<void> => {
    ...
    try {
      const urlPath = getEndpoint(process.env.TASK_TYPE_DELETE_ENDPOINT as string)
      const options: Partial<FetchOptions> = {
        method: 'DELETE',
        pathParams: { task_type_id: id },
      }
